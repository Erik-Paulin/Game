...
def draw(player, elapsed_time, projs, store, days):
    WIN.blit(BG, (0, 0))
    ...
    day_text = FONT.render(f"Days: {round(elapsed_time)}s", 1, "white")
    WIN.blit(day_text, (10, 20))
...
def stock(store_change, elapsed_time, days):
    if int(elapsed_time)/10 > days:
        store_change = random.randint(0.75,1.25)


def day(days,elapsed_time):
    if int(elapsed_time)/10 > days:
        days += 1
...
def main(start_time):
    ...
    while run:
        ...
        day(days,elapsed_time)

        draw(player, elapsed_time, projs, store, days)
